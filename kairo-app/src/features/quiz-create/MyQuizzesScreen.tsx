...
export function MyQuizzesScreen() {
  ...
  const { status, data, error, retry, setData } = useAsyncResource(
    () => getMyQuizzes(),
    { fallbackError: 'Could not load your quizzes.' },
  );
  const quizzes = data ?? [];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: '100dvh', fontFamily: 'var(--font-body)', background: 'var(--dark-bg-canvas)' }}>
      ...
      <div style={{ padding: '16px 20px 40px', flex: 1 }}>
        ...
        {!studentId ? (
          ...
          <KairoScreenState status={status} inline message="Loading your quizzes…" errorMessage={error} onRetry={retry} />
        ) : quizzes.length === 0 ? (
