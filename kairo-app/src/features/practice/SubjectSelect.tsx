  ...
  return (
    <div style={{
      ...
      <button type="button" onClick={onToggleFav} aria-label={favourite ? `Remove ${s.label} from favourites` : `Add ${s.label} to favourites`} aria-pressed={favourite} style={{
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill={favourite ? 'var(--kairo-gold-500)' : 'none'} stroke={favourite ? 'var(--kairo-gold-600)' : 'var(--dark-text-faint)'} strokeWidth="2"><path d="M12 17.3l-6.2 3.6 1.6-7-5.4-4.7 7.1-.6L12 2l2.9 6.6 7.1.6-5.4 4.7 1.6 7z" /></svg>
      </button>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, fontFamily: 'var(--font-body)', background: 'var(--dark-bg-canvas)' }}>
      ...
      <div style={{ padding: '10px 20px 0', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 'var(--radius-md)', border: '1.5px solid var(--dark-border)', marginBottom: 20 }}>
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search subjects" style={{
            border: 'none', outline: 'none', flex: 1, fontSize: 14, fontFamily: 'var(--font-body)', color: 'var(--dark-text-heading)', background: 'transparent',
          }} />
        </div>

        {!query && recents.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--dark-text-muted)', letterSpacing: '.02em', marginBottom: 8 }}>RECENT</div>
        ...
        {!query && favSubjects.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--dark-text-muted)', letterSpacing: '.02em', marginBottom: 8 }}>FAVOURITES</div>
      ...
      <div style={{ padding: '0 20px 24px' }}>
        <Button variant="darkAccent" size="lg" fullWidth disabled={!selected} onClick={() => { const s = subjects.find((s) => s.key === selected); if (s) onPick(s); }}>Continue</Button>
      </div>
    </div>
  );
}
