...
Usage (from kairo-learning-engine/):
...
"application", "analysis", "synthesis" -- "recall" is a bare fact/definition; \
"application" uses understanding to work through a new instance of a known type of \
problem; "analysis" requires distinguishing between similar concepts or spotting what \
a question is really asking versus what it seems to ask; "synthesis" requires \
connecting multiple pieces of given information before solving).
...
def learning_objective(subtopic, exam_body, year):
    # Matches the established convention already used across the live
    # content (e.g. "Answer a real JAMB 2004 question on Industrial
    # electrolysis (chlor-alkali process)."), not an independently
    # invented template.
    if year:
        return f"Answer a real {exam_body} {year} question on {subtopic}."
    return f"Answer a real {exam_body} question on {subtopic}."
...
def to_qim_question(item, raw_item, exam_body, year):
    ...
    return {
        ...
        "learningObjective": learning_objective(item["subtopic"], exam_body, year),
        "conceptsTested": [],
